...
class DataManager(object):
    ...
    def writeResultsFromNumpyLabel(self,result,key):
        ...
        image1 = np.transpose(sitk.GetArrayFromImage(toWrite).astype(dtype=float), [2, 1, 0])
        #image1 = np.transpose(sitk.GetArrayFromImage(toWrite).astype(dtype=float), [1, 2, 0])
        utilities.sitk_show(image1)
